...
def main():
    ...
    result = []
    for f in files:
        result.append(LR_modeling('field/' + f, 30))

    # load target word id list
    match_list = pickle.load(open("target_word.p", "rb"))
    # find real ngram in each legal fiels
    for k, f in enumerate(files):
        pos_list = list(result[k][0])
        neg_list = list(result[k][1])
        pos_list = map(lambda x: int(x), pos_list)
        neg_list = map(lambda x: int(x), neg_list)
        print f
        print k, 'pos'
        for i in range(len(match_list)):
            if match_list[i][1] in pos_list:
                print match_list[i]
        print k, 'neg'
        for i in range(len(match_list)):
            if match_list[i][1] in neg_list:
                print match_list[i]
